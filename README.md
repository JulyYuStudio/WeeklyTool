...
在编辑器中选中文本,右键选择 "Base64加密" 或 "Base64解密" 来进行编码/解码操作。

#### 特性

- 仅在选中文本时生效
- 支持批量操作,可以同时处理多个选中区域
- 自动在选中文本周围添加必要的换行符,保持格式不变
...
3. 选择 "Base64加密" 或 "Base64解密"

4. 自动在选中文本周围添加换行符,并进行相应的编码/解码操作

#### 注意事项

- 输入的文本必须是合法的 Base64 编码字符串,否则解码时会报错

**实现文件**: [src/base64Tools.ts](src/base64.ts)
